...
import { joinInviteLink, joinSearchResult, leaveFeedChannel, previewInviteLink } from "./telegramMembership";
...
test("previews Telegram invite links", async () => {
  ...
  const preview = await previewInviteLink("https://t.me/+preview_hash", async () => client as never);
...
test("joins invite links and returns the joined chat entity", async () => {
  ...
  const entity = await joinInviteLink("tg://join?invite=joined_hash", async () => client as never);
...
test("leaves channels with channels.LeaveChannel", async () => {
  ...
  expect(client.invoke).toHaveBeenCalledWith(
    expect.objectContaining({ channel: { className: "InputPeerChannel", channelId: 10 } }),
  );
});
