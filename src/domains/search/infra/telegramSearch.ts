...
type SearchPeer = Api.TypePeer | { className?: unknown; userId?: unknown; channelId?: unknown; chatId?: unknown };
type SearchChat = Api.TypeChat | { className?: unknown; id?: unknown; title?: unknown; username?: unknown; broadcast?: unknown };
type SearchUser = Api.TypeUser | { className?: unknown; id?: unknown; firstName?: unknown; lastName?: unknown; username?: unknown };
...
function getPeerClassName(peer: SearchPeer): string | null {
  return typeof peer === "object" && peer && "className" in peer && typeof peer.className === "string"
    ? peer.className
    : null;
}
...
function getPeerKey(peer: SearchPeer): string | null {
  ...
  if (className === "PeerChannel") {
    const channelId = getIdValue("channelId" in peer ? peer.channelId : undefined);
...
function toSearchResultFromPeer(
  ...
  if (className === "PeerChannel") {
    ...
    return {
      ...
      username: "username" in chat && typeof chat.username === "string" ? chat.username : undefined,
      channelKey: `group:${id}`,
...
export async function searchTelegram(
  ...
  const peerResults = [...(found.myResults ?? []), ...(found.results ?? [])]
    ...
    .filter((result, index, array) => array.findIndex((entry) => entry.id === result.id && entry.kind === result.kind) === index);
  ...
  const messageMaps = getEntityMaps(globalMessages.chats ?? [], globalMessages.users ?? []);
  const messageResults = (globalMessages.messages ?? [])
    .filter(
      (message): message is Api.Message =>
