...
function getInviteTitle(invite: unknown): string {
  if (invite && typeof invite === "object" && "title" in invite && typeof invite.title === "string") {
    return invite.title;
  }
  return "Invite";
}
...
function getInviteKind(invite: unknown): "channel" | "group" {
  if (invite && typeof invite === "object" && "broadcast" in invite && invite.broadcast === true) {
    return "channel";
  }
  return "group";
}
...
export async function previewInviteLink(
  ...
  const entity =
    invite && typeof invite === "object" && "chat" in invite ? invite.chat : undefined;
...
export async function joinInviteLink(
  ...
  if (updates && typeof updates === "object" && "chats" in updates && Array.isArray(updates.chats)) {
...
export async function leaveFeedChannel(
  ...
  const client = await ensureTelegramConnected();
  const source = item.sourceMessage as { getInputChat?: () => Promise<unknown>; inputChat?: unknown } | undefined;
  const inputChat = source?.getInputChat ? await source.getInputChat() : source?.inputChat;

  if (inputChat && typeof inputChat === "object" && "className" in inputChat && inputChat.className === "InputPeerChat") {
    const fallbackChatId = item.channelKey ? Number.parseInt(item.channelKey.split(":")[1] ?? "0", 10) : 0;
    const chatId =
      "chatId" in inputChat && typeof inputChat.chatId === "number" ? inputChat.chatId : fallbackChatId;
    await client.invoke(
