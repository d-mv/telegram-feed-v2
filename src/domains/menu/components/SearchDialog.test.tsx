...
test("searches Telegram and opens the selected result in thread view", async () => {
	...
	await waitFor(() => {
		expect(searchTelegramMock).toHaveBeenCalledWith(
			"joined",
			ensureTelegramConnected,
		);
	});
	...
	await user.click(
		screen.getByRole("button", { name: /Open Joined Channel/i }),
	);
...
test("shows join for unjoined results and joins before opening", async () => {
	...
	joinSearchResultMock.mockResolvedValue({
		className: "Channel",
		id: 88,
		title: "Public Channel",
		username: "public",
...
test("shows invite preview and joins invite links", async () => {
	...
	joinInviteLinkMock.mockResolvedValue({
		className: "Channel",
		id: 12,
		title: "Invite Group",
	});
	...
	await user.type(
		screen.getByLabelText("Search Telegram"),
