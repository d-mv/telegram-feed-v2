...
export default function SearchDialog() {
  ...
  useEffect(() => {
    ...
    previewInviteLink(trimmed, ensureTelegramConnected)
      .then((preview) => {
        ...
        return searchTelegram(trimmed, ensureTelegramConnected).then((nextResults) => {
  ...
  async function handleSelect(result: SearchResult) {
    try {
      ...
      setNotificationFocus({ channelKey: item.channelKey, itemId: item.id, view: "thread" });
      closeMenu();
    } catch {
  ...
  return (
    <MenuDialog title="Search" onClose={closeMenu}>
      <Flex vertical gap={12}>
        ...
        {query.trim() === "" && (
          <Typography.Text type="secondary">Search results will appear here.</Typography.Text>
        )}
        {invitePreview && (
          <Flex vertical gap={6} style={{ padding: "10px 12px", border: "1px solid var(--ant-color-border)", borderRadius: 8 }}>
            <Typography.Text strong>{invitePreview.title}</Typography.Text>
            <Typography.Text type="secondary">
              {invitePreview.kind === "channel" ? "Channel" : "Group"} · {invitePreview.participantsCount} members
            </Typography.Text>
            ...
            renderItem={(result) => (
              <List.Item
                ...
                extra={
                  <Button
                    size="small"
                    type={result.isJoined || result.kind === "direct" ? "default" : "primary"}
                    aria-label={`${result.isJoined || result.kind === "direct" ? "Open" : "Join"} ${result.title}`}
                  >
                    {result.isJoined || result.kind === "direct" ? "Open" : "Join"}
                  </Button>
                }
              >
                <List.Item.Meta
                  ...
                  description={<Typography.Text type="secondary" style={{ fontSize: 12 }}>{getResultDescription(result)}</Typography.Text>}
                />
              </List.Item>
            )}
          />
