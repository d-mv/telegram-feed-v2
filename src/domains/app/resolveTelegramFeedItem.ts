...
export function parseTelegramRoute(
	url: string,
): { url: string } | "unsupported" {
	try {
		...
		if (parsed.protocol === "tg:") {
			...
			return {
				url: post ? `https://t.me/${domain}/${post}` : `https://t.me/${domain}`,
			};
		}
...
export async function resolveTelegramFeedItem(
	...
	if (typeof entityRef === "string") {
		...
		if (route !== "unsupported") {
			...
			targetMessageId =
				targetMessageId ?? parseTelegramMessageId(segments[1] ?? null);
		}
	}

	const entity =
		typeof targetRef === "string"
			? await client.getEntity(targetRef)
			: targetRef;
	const messages = await client.getMessages(
		entity,
		targetMessageId ? { ids: [targetMessageId] } : { limit: 1 },
	);
	const message = messages.find(
		(entry): entry is Api.Message => entry instanceof Api.Message,
	);
	if (!message) {
