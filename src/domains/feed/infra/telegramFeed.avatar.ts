...
type UserPhotosRequest = ConstructorParameters<typeof Api.photos.GetUserPhotos>[0];
...
export async function getAvatarPhotoUrl(
  ...
  const task = (async () => {
    try {
      const gallery = await getAvatarPhotoGallery(entity, cacheKey, ensureTelegramConnected);
      if (gallery.length > 0) {
      ...
      const photo = await client.downloadProfilePhoto(entity as ProfilePhotoEntity, { isBig: false });
  ...
  const task = (async () => {
    try {
      ...
      for (const photo of photos) {
        ...
        const binary = await client.downloadMedia(photo as unknown as DownloadMediaTarget, {});
