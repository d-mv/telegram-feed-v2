...
vi.mock("../infra/telegramFeed", async () => {
	const actual = await vi.importActual<typeof import("../infra/telegramFeed")>(
		"../infra/telegramFeed",
	);
	return {
		...actual,
...
test("loads message comments in accordion and shows comments icon", async () => {
	...
	ensureTelegramConnectedMock.mockResolvedValue({
		getMessages: vi
			.fn()
			.mockImplementation((_chat: unknown, options?: { replyTo?: number }) => {
...
test("groups consecutive media-only messages from the same sender in the thread", async () => {
	...
	expect(
		document.querySelector('[data-media-group-layout="image-grid"]'),
	).toBeTruthy();
	expect(
		document.querySelectorAll('[data-media-group-tile="true"]'),
	).toHaveLength(2);
	expect(document.querySelector('[data-media-type="image"]')).toHaveStyle({
		aspectRatio: "1 / 1",
	});
});
...
test("loads full thread history for a cached item without sourceMessage", async () => {
	...
	const getMessages = vi
		.fn()
		.mockImplementation((chat: unknown, options?: { ids?: number[] }) => {
			if (options?.ids) {
...
test("groups a media-only multi-image thread message with adjacent media-only messages", async () => {
	...
	ensureTelegramConnectedMock.mockResolvedValue({
		getMessages: vi
			.fn()
			.mockResolvedValue([thirdMessage, secondMessage, firstMessage]),
	});
...
test("renders forwarded metadata inside the thread and routes clicks internally", async () => {
	...
	const badge = screen.getByRole("button", {
		name: "Forwarded from From anonymous via News",
	});
