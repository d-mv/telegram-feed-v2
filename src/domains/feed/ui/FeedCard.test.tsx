...
test("renders grouped images in a tiled row layout", () => {
	...
	const grid = container.querySelector(
		'[data-media-group-layout="image-grid"]',
	);
	expect(grid).toBeTruthy();
	expect(grid?.querySelectorAll('[data-media-group-tile="true"]')).toHaveLength(
		3,
	);
...
test("renders a single message with multiple images as a gallery", () => {
	...
	expect(
		container.querySelector('[data-media-group-layout="image-grid"]'),
	).toBeTruthy();
	expect(
		container.querySelectorAll('[data-media-group-tile="true"]'),
	).toHaveLength(3);
});
...
test("renders forwarded source metadata and opens resolvable forwarded targets internally", async () => {
	...
	expect(
		screen.getByText("Forwarded from From anonymous via News"),
	).toBeInTheDocument();
	await user.click(
