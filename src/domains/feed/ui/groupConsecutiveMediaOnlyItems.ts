...
function isMediaOnlyItem(item: FeedItem): boolean {
  return (Boolean(item.media) || (item.mediaItems?.length ?? 0) > 0) && item.text.trim() === "";
}
...
export function groupConsecutiveMediaOnlyItems(items: FeedItem[]): FeedItem[][] {
  ...
  for (const item of items) {
    ...
    if (currentGroup && previousItem && shouldGroupTogether(previousItem, item)) {
