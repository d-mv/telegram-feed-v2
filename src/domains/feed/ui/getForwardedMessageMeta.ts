...
export function getForwardedMessageMeta(
	sourceMessage: unknown,
): ForwardedMessageMeta | null {
	if (!sourceMessage || typeof sourceMessage !== "object") {
		return null;
	}

	const forward =
		"forward" in sourceMessage &&
		sourceMessage.forward &&
		typeof sourceMessage.forward === "object"
			? sourceMessage.forward
			: undefined;
	const fwdFrom =
		"fwdFrom" in sourceMessage &&
		sourceMessage.fwdFrom &&
		typeof sourceMessage.fwdFrom === "object"
			? sourceMessage.fwdFrom
			: undefined;

	const forwardedChat = forward && "chat" in forward ? forward.chat : undefined;
	const forwardedSender =
		forward && "sender" in forward ? forward.sender : undefined;
	const channelName = getEntityLabel(
		forwardedChat,
		fwdFrom && "fromName" in fwdFrom && typeof fwdFrom.fromName === "string"
			? fwdFrom.fromName
			: "",
	);
	...
	const anonymousAuthor =
		fwdFrom &&
		"postAuthor" in fwdFrom &&
		typeof fwdFrom.postAuthor === "string" &&
		fwdFrom.postAuthor !== ""
			? "From anonymous"
			: "";
	...
	if (channelUsername) {
		const channelPost =
			fwdFrom &&
			"channelPost" in fwdFrom &&
			typeof fwdFrom.channelPost === "number"
				? fwdFrom.channelPost
				: undefined;
		return {
			label,
