...
export function Poll({ item }: PollProps) {
  ...
  return (
    ...
    >
      <Typography.Title level={5} style={{ marginTop: 0, marginBottom: 12, fontSize: 15 }}>
        {poll.question}
      </Typography.Title>

      <Flex vertical gap={12}>
        {poll.options.map((option: PollOption) => {
          const isSelected = selectedOptions.some((o) => o.toString() === option.option.toString());
          const percentage = poll.totalVoters > 0 ? Math.round((option.votersCount / poll.totalVoters) * 100) : 0;
              ...
              {showResults ? (
                <div style={{ position: "relative" }}>
                  <Flex justify="space-between" align="center" style={{ marginBottom: 4 }}>
                    <Typography.Text strong={option.chosen} style={{ fontSize: 13 }}>
                      {option.text}
                  ...
                  <Progress
                    percent={percentage}
                    showInfo={false}
                    strokeColor={option.chosen ? token.colorPrimary : token.colorFillSecondary}
                    size="small"
                  />
                </div>
              ) : (
                ...
                >
                  ...
                  <Typography.Text style={{ fontSize: 13 }}>{option.text}</Typography.Text>
                </div>
              )}
            </div>
          );
