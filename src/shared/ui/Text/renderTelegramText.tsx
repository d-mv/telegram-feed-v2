...
function renderEntity(
  ...
  switch (entity.className) {
    ...
    case "MessageEntityPre":
      return (
        <pre key={key} data-language={typeof entity.language === "string" ? entity.language : undefined}>
          <code>{value}</code>
        </pre>
      );
...
export function renderTelegramText(
  ...
  const normalized = entities
    .map((entity) => (entity && typeof entity === "object" ? (entity as TelegramEntity) : null))
    ...
    .filter((entity): entity is { entity: TelegramEntity; start: number; end: number } => entity !== null)
  ...
  for (const segment of normalized) {
    ...
    if (segment.start > cursor) {
      nodes.push(<span key={`text-${cursor}`}>{text.slice(cursor, segment.start)}</span>);
    }

    const value = text.slice(segment.start, segment.end);
    nodes.push(renderEntity(`entity-${segment.start}-${segment.end}`, value, segment.entity));
    cursor = segment.end;
  }
